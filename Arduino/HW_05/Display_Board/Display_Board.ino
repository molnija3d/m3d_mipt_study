...
void setup() {
  Wire.begin(1);  // подключиться к шине i2c с адресом #4
...
void loop() {
  ...
  if (data_ok) {
    Serial.println("\n------ Recieved data: -------");
    Serial.println((String) "Time = " + sensors.data.time + "ms - " + sizeof(sensors.data.time) + " bytes");
    Serial.println((String) "Temp = " + sensors.data.temp + "C - " + sizeof(sensors.data.temp) + " bytes");
    Serial.println((String) "Hum = " + sensors.data.hum + "% - " + sizeof(sensors.data.hum) + " bytes");
    Serial.println((String) "Light = " + sensors.data.light + " lux - " + sizeof(sensors.data.light) + " bytes");
